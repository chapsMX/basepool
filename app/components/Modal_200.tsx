...
export default function PoolModal({ isOpen, onClose }: PoolModalProps) {
  ...
  }) as { data: [bigint, bigint, bigint, bigint] | undefined; isLoading: boolean; error: Error | null };

  // Añadir logging para debug
  console.log('Pool Status Data:', data);
  ...
  // Verificar si hay error y mostrar más detalles
  if (error) {
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 w-[90%] max-w-[400px] mx-4">
          ...
          <div className="text-center py-4 [font-family:ProtoMono] text-red-500">
            Error loading pool status: {error.message}
          </div>
        </div>
      </div>
    );
  ...
  const threshold = data?.[3] ? Number(formatEther(data[3])) : 0.1;
        ...
        {isLoading ? (
          ...
          <>
            ...
            {conqueredNumbers && conqueredNumbers.length > 0 && (
              <div className="mt-4">
                <div className="text-gray-500 mb-2">Previous Winners:</div>
                <div className="flex flex-wrap gap-2">
